  ...
  const updateStats = (stars: number) => {
    ...
    const newProfile = {
      ...
      stats: {
        ...
        dayStreak: Math.max(1, userProfile.stats.dayStreak)
      }
    };
      ...
      <div className="flex flex-col min-h-screen bg-slate-50 text-slate-900 pb-20">
        <header className="bg-white border-b border-slate-200 sticky top-0 z-50 p-4 flex justify-between items-center shadow-sm">
          ...
          <div className="flex items-center gap-1 bg-amber-50 px-3 py-1 rounded-full border border-amber-100">
            ...
            <span className="font-black text-amber-700 text-xs">{userProfile.stats.totalStars}</span>
          </div>
        </header>
        ...
        <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 flex justify-around items-center p-3 z-50 shadow-lg rounded-t-[2.5rem]">
